...
rawurl = 'https://raw.githubusercontent.com/usgs/gems-tools-arcmap/master/gems-toolbox/GeMS_RelationshipClasses1_Arc10.py'
...
def main(parameters):
    ...
    Glossary = os.path.join(inGdb, 'Glossary')
    GeoMat = os.path.join(inGdb, 'GeoMaterialDict')
    CAF = os.path.join(inGdb, 'GeologicMap', 'ContactsAndFaults')
    MUP = os.path.join(inGdb, 'GeologicMap', 'MapUnitPolys')
    ORP = os.path.join(inGdb, 'GeologicMap', 'OrientationPoints')
    GEL = os.path.join(inGdb, 'GeologicLines')
    ...
    if not arcpy.Exists(os.path.join(inGdb, 'RelationshipClasses')):
