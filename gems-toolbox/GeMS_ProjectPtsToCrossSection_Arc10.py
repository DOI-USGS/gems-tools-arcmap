...
def main(parameters):
    ...
    rawurl = 'https://raw.githubusercontent.com/usgs/gems-tools-arcmap/master/gems-toolbox/GeMS_ProjectPtsToCrossSection_Arc10.py'
    ...
    if debug:
      for i in range(0,len(parameters)):
        addMsgAndPrint(str(i)+' '+str(parameters[i]))

    sectionLineClass = parameters[0]
    sectionLineLabelValue = parameters[1]
    pointClass = parameters[2]
    idField = parameters[3]
    vertEx = float(parameters[4])
    DEM = parameters[5]
    maxDistance = parameters[6]
    outWorkspace = parameters[7]
    outFeatureClass = parameters[8]
    if parameters[9] <> '#':
        tempWorkspace = parameters[9]
    else:
        tempWorkspace = outWorkspace
     
    tempBuff =  os.path.join(tempWorkspace, 'xxx1')     # temporary Poly fc created by Buffer
    tempPoints = os.path.join(tempWorkspace, 'xxx2')    # temporary Point fc created by Clip
    secLine = 'xxx3'
    secLinePts = os.path.join(tempWorkspace, 'xxx4')
    ...
    delArcStuff((tempBuff, tempPoints, secLinePts, os.path.join(outWorkspace, outFeatureClass))
    ...
    newFC = os.path.join(outWorkspace, outFeatureClass)
    # add basic fields
